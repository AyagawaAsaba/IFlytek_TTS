...
class Ws_Param(object):  # 定义类用于管理 websocket 参数
    # 初始化
    def __init__(self, API_ID, API_Key, API_Secret, Text):
        self.API_ID = API_ID
        self.API_Key = API_Key
        self.API_Secret = API_Secret
        self.Text = Text  # 需语音合成的文本
        # 公共参数(common)
        self.CommonArgs = {"API_ID": self.API_ID}
        # 业务参数(business)
        self.BusinessArgs = {
            "aue": "lame",  # 音频编码 lame mps格式
            "sfl": 1,  # 开启流式返回
    ...
    def create_url(self):
        ...
        # step2 结合API_Secret对signature_origin，使用hmac-sha256进行加密（签名）
...
def tts_api_get_result(API_ID, API_Key, API_Secret, TEXT, tts_file):
    # 实例化websocket参数类
    wsParam = Ws_Param(API_ID=API_ID,
                       API_Key=API_Key,
